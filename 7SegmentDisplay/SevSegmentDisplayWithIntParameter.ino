...
// Test function to showcase all four behaviors
void testSevenSegmentDisplay() {
    SevenSegmentDisplay display;

    // Test cases
    display.displayNumber(25); // Case a: num > 19 (DOT ON, other segments OFF)
    delay(2000);

    display.displayNumber(-5); // Case b: num < 0 (Display OFF)
    delay(2000);

    display.displayNumber(7); // Case c: num in [0, 9] (Display 7)
    delay(2000);

    display.displayNumber(15); // Case d: num in [10, 19] (Display 5 with DOT ON)
    delay(2000);
}
